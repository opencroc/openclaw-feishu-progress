...
export class FileFeishuWebhookDedupStore extends InMemoryFeishuWebhookDedupStore {
  ...
  private trim(): void {
    const sorted = [...this.entries.entries()].sort((left, right) => right[1] - left[1]).slice(0, this.maxEntries);
    this.entries.clear();
  ...
  private load(): void {
    ...
    try {
      ...
      for (const entry of entries) {
        if (!entry || typeof entry.key !== 'string' || typeof entry.expiresAt !== 'number') continue;
  ...
  private save(): void {
    const data: SerializedWebhookDedupFile = {
      version: 1,
      entries: [...this.entries.entries()].map(([key, expiresAt]) => ({ key, expiresAt })),
    };
